...
class gule_recent_posts extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'gule-recent-posts', // Base widget ID
			__('01 : Recent Posts Thumbnail', 'gule'), // Title of Widget
			array( 
				'description' => __( 'Display recent blog posts with Thumbnails.', 'gule' ),
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<?php
         ...
         ?>
        <div class="gule-recent-posts media">
	         ...
	         <div class="recent-post-body media-body">
	         	...
	         	<?php do_action('gule_recent_post_widget_meta'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Posts', 'gule' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'gule' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'gule' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'gule' ); ?></label>
...
add_action( 'widgets_init', 'register_gule_widgets' );  
function register_gule_widgets() {  
    register_widget( 'gule_recent_posts' );  
} 